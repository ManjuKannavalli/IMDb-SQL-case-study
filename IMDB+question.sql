...
-- There are null values in movie table in columns: 
-- country(20),worldwide_gross_income(3724),languages(194), and production_company(528)
...
-- Number of movies released  by the month
...
-- in 2019, the movies produced in either USA or India were 1059.
...
SELECT DISTINCT ROW_NUMBER() over() as row_num, genre
FROM genre
group by genre;

-- 13 genres are available in genre table
...
from movie
inner join genre where genre.movie_id = movie.id
group by genre
order by number_of_movies desc limit 1;

-- '4285' movies in Drama genre is the highest in the list.
...
-- 3289 movies in the dataset has only one genre listed
...
-- Action genre has the longest duration of 112.88 minutes
-- Sci-fi and horror fall under 100 minutes of avg_duaration
...
-- Dream warrior pictures, and National theatre Live has produced most number of hit movies.
...
-- in March 2017 release, 24 Drama movies had more than 1000 votes
...
--  There are 8 movies which has 'The' in the beginning of title
...
-- 361 movies are released in the period of April 2018 to April 2019 which has median rating of 8
...
-- James Mangold, Anthony Russo, Soubin Ahahir are the top 3 directors from top 3 genres
...
+---------------+-------------------+---------------------+----------------------+-----------------+*/
-- Type your code below:
SELECT names.NAME                                                           AS
       actor_name,
       ratings.total_votes                                                    AS
       total_votes,
       Count(ratings.movie_id)                                                AS
       movie_count,
       Round(Sum(ratings.avg_rating * ratings.total_votes) / Sum(ratings.total_votes), 2) AS
       actor_avg_rating,
       Rank()
         OVER(
           ORDER BY Round(Sum(ratings.avg_rating * ratings.total_votes) / Sum(ratings.total_votes), 2)
         DESC)
                                                                        AS
       actor_rank
FROM   names
...
-- Star Cinema and Twentieth Century Fox are top 2 production houses.
